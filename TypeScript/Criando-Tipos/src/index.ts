interface GithubUserResponse { //Interface que recebe os dados da API do Github 
   id: number
   login: string
   name: string
   bio: string
...
async function fetchUser(username: string) {
   ...
   if (user.message) {
      ...
      console.log(
         ...
         `\nNome: ${user.name}` +
         `\nBio: ${user.bio}` +
         `\nRepostórios públicos: ${user.public_repos}` 
      )
   }
}
...
function showReposTotal() {
   ...
   console.log(`O grupo possui um total de ${reposTotal} respositórios públicos`)
}

function showTopFive() {
   const topFive = users.slice().sort((a, b) => b.public_repos - a.public_repos).slice(0, 5)

   let message = `Top 5 Usuarios com mais repositórios públicos:\n`
